...
class PDFParser(DocumentParser):
    ...
    - Table -> structured rows/columns + Markdown
    - Picture/chart/diagram -> Docling annotations
    - Scanned/image-based page -> OCR text

    Document identity:
    - SHA-256 is calculated from source PDF bytes
    - document_id uses the same SHA-256
    - Same physical PDF therefore keeps the same identity
      across partial page-range runs
    """

    PARSER_NAME = (
        "docling-adaptive-pdf-parser"
    )
    ...
    def _build_ocr_converter(
        ...
        Called only for pages identified
        as OCR candidates during Pass 1.
        """
    ...
    def can_parse(
        ...
        """
        Return True when supplied path
    ...
    def calculate_hash(
        ...
        """
        Calculate SHA-256 from actual PDF bytes.
        """
    ...
    def parse(
        ...
        Pass 1:
            native layout/content detection

        Pass 2:
            OCR only on candidate pages

        document_id is deterministic and based on
        document SHA-256.
        ...
        still produce the same document_id.
        """
        ...
        # Same physical PDF bytes = same logical document.
        document_id = (
            document_hash
        )
        ...
        try:
            ...
            pictures = (
                self._extract_pictures(
                    document
                )
            )

            pages = (
                self._build_detection_pages(
                    document=document,
                    blocks=blocks,
                    tables=tables,
                    pictures=pictures,
            ...
            warnings = (
                self._generate_document_warnings(
                    pages=pages,
                    tables=tables,
                    pictures=pictures,
                )
            ...
            return {
                ...
                "document_metadata": {
                    # For a partial parser run,
                    # this is the number of pages
                    # returned by this conversion.
                    "page_count": (
                        total_pages
                    ),
    ...
    def _build_detection_pages(
        ...
        """
        Build Pass-1 page representation.
        """
    ...
    def _bbox(
        ...
        """
        Serialize Docling bounding box.
        """
    ...
    def _item_label(
        ...
        """
        Return normalized Docling label.
        """
    ...
    def _item_reference(
        ...
        """
        Return Docling self reference.

        Note:
        This identifier is only stable inside
        the individual Docling conversion.

        Document-level merge logic therefore
        replaces items based on page ownership
        rather than globally trusting this ID.
        """
    ...
    def _quality_score(
        ...
        """
        Calculate basic page-coverage score.

        Note:
        This is NOT semantic extraction accuracy.
        Real accuracy is measured by the
        evaluation/golden-set component.
        """
    ...
    def _generate_document_warnings(
        ...
        if unclassified_pictures:
            ...
            warnings.append(
                (
                    f"{len(unclassified_pictures)} "
                    "picture(s) were detected "
                    "without a confident type."
                )
            )
